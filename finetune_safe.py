...
def load_model(model_dir):
    ...
    # Freeze embedding layers (only train attention + dense + RNN)
...
class PairDataset:
    def __init__(self, db_path, max_pairs=20000):
        ...
        self.pairs = [(a, b, 1) for a, b in true_pairs] + [
            (a, b, 0) for a, b in false_pairs
        ]
        random.Random(42).shuffle(self.pairs)
        print(
            f"  {len(true_pairs):,} true + {len(false_pairs):,} false = {len(self.pairs):,} total"
        )
        ...
        for i in range(0, len(all_ids), 900):
            ...
            cur.execute(
                f"SELECT id, instructions_list FROM filtered_functions WHERE id IN ({placeholders})",
                batch,
            )
...
def main():
    ...
    if len(args) < 1:
        print(
            "Usage: finetune_safe.py <db_path> [model_dir] [output_path] [max_pairs] [epochs] [batch_size]"
        )
        sys.exit(1)
    ...
    output_path = (
        args[2] if len(args) > 2 else os.path.join(model_dir, "SAFEtorch_finetuned.pt")
    )
    ...
    print(
        f"  {frozen:,} frozen (embedding+RNN), {trainable:,} trainable (attention+dense)"
    )
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, n, batch_size):
            ...
            if n_batches % 25 == 0:
                ...
                print(
                    f"  Ep {epoch+1}/{epochs}  batch {n_batches}/{n//batch_size+1}  loss={total_loss/n_batches:.6f}  {elapsed:.0f}s  est {remaining:.0f}s",
                    flush=True,
                )

        print(
            f"  Epoch {epoch+1} done  avg_loss={total_loss/n_batches:.6f}  [{time.time()-t0:.0f}s]"
        )

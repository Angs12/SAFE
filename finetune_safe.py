...
class PairDataset:
    def __init__(self, db_path, max_pairs=20000):
        ...
        for r in cur.fetchall():
            if r[2] == 1:
                true_pairs.append((r[0], r[1]))
            else:
                false_pairs.append((r[0], r[1]))
        n_true = len(true_pairs)
        if len(false_pairs) > n_true:
            random.Random(42).shuffle(false_pairs)
            false_pairs = false_pairs[:n_true]
        if max_pairs and n_true > max_pairs:
            random.Random(42).shuffle(true_pairs)
            true_pairs = true_pairs[:max_pairs]
            false_pairs = false_pairs[:max_pairs]
        ...
        for i in range(0, len(all_ids), 900):
            ...
            cur.execute(f"SELECT id, instructions_list FROM filtered_functions WHERE id IN ({','.join('?' * len(batch))})", batch)
